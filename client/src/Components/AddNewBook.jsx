    ...
    <div className='bg-[#f1ead7] min-h-screen flex flex-col justify-center items-center'>
      ...
      <div className='mx-auto p-10 mb-10 max-w-3xl grid grid-cols-3 gap-8 bg-white/60 rounded-2xl'>
        ...
        <div className='col-span-1 flex flex-col pt-6'>
          ...
          <label className="relative w-full h-68 border-2 border-dashed border-gray-300 rounded-2xl flex flex-col items-center justify-center cursor-pointer hover:bg-white/40 overflow-hidden">
            ...
            {/* Ajout d'une condition pour ne pas afficher l'image vide et de rounded-2xl pour respecter les bords */}
            {preview && <img id="image-preview" src={preview} alt="Aperçu" className="absolute inset-0 w-full h-full object-cover rounded-2xl" />}
          </label>
        </div>
        ...
        <form className='rounded-2xl space-y-5 p-6 col-span-2 min-w-0' onSubmit={handleSubmit}>
          ...
          {/* SECTION GENRE NETTOYÉE ET CORRIGÉE */}
          <div>
          ...
          {/* CHAMP CUSTOM GENRE AFFICHÉ DYNAMIQUEMENT */}
          {formData.genre === 'Others' && (
            <div>
              ...
              <input 
                ...
                type="text"
                placeholder='Type in the genre if you selected other' 
                className='bg-[#FFFBF2] border border-[#EFE7D6] rounded-lg w-full py-1 focus:outline-none text-[#7A6A5A] pl-1.5 placeholder:text-[#e6cbb2] placeholder:font-extralight placeholder:font-sans' 
              />
            </div>
          )}
